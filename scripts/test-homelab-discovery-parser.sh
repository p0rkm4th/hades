...
result = parse_nmap_xml(xml, target="192.0.2.0/29", allowed_networks=["192.0.2.0/29"])
...
for target in ("10.0.0.0/8", "192.0.2.0/28"):
    try:
        parse_nmap_xml(xml, target=target, allowed_networks=["192.0.2.0/29"])
...
try:
    parse_nmap_xml(out_of_scope, target="192.0.2.0/29", allowed_networks=["192.0.2.0/30"])
except ValueError as exc:
    assert "scope" in str(exc)
else:
...
for bad in (b"not xml", b"", b"x" * (2 * 1024 * 1024 + 1)):
    try:
        parse_nmap_xml(bad, target="192.0.2.0/29", allowed_networks=["192.0.2.0/29"])
    except ValueError:
        pass
    else:
