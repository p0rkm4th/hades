...
searx_line=$(grep -nF '"${compose_cmd[@]}" -f "$HADES_SEARXNG_COMPOSE_FILE" up -d' "$installer" | cut -d: -f1)
hermes_line=$(grep -nF 'install -m 0600 "$HADES_HERMES_SERVICE_FILE"' "$installer" | cut -d: -f1)
