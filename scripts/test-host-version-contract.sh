...
grep -q 'at least 2 CPU cores are required' "$installer" || { echo 'FAIL minimum CPU is not enforced'; exit 1; }
grep -q 'at least 8 GiB RAM is required' "$installer" || { echo 'FAIL minimum RAM is not enforced'; exit 1; }
