...
- **Active independent priority:** finish the unblocked homelab acceptance
  ...
  NetBox, and Kuma path is exercised; physical host recovery, GPU runtime
  qualification, DNS normalization, and the seventh-host search remain open.
...
- **Open P0/P1/P2:** no newly demonstrated P0/P1/P2 defect is recorded; the
  owner-visible composition on a reconstructed deployment remains open.
...
- **Next independent action:** continue physical homelab acceptance from the
  current evidence: recover reachable host state where available, qualify
  hardware without destructive operations, normalize observed identity, and
  keep the HADES read path source-aware. Do not promote Hermes or create a
  fragile two-node Proxmox cluster.
...
homelab authority fixture. The current tree
...
- Deterministic automation preparation is complete. A private n8n runner now
  hosts one inactive read-only canary, with live bounded metadata projection
  verification and no new control authority. The proposed boundary requires
  trusted subject/capability resolution, explicit preview/apply semantics,
  idempotency, canonical verification, and fail-closed unknown outcomes;
  workflow enablement remains owner-gated.
