...
| `HADES_OPEN_WEBUI_COMPOSE_FILE`, `HADES_HINDSIGHT_COMPOSE_FILE`, `HADES_SEARXNG_COMPOSE_FILE`, `HADES_HERMES_SERVICE_FILE` | installer | yes | paths to four private runtime records, mode 0600 or 0640; Hindsight and SearXNG records must use the immutable references in `config/versions.env` | installer/root | source-controlled templates or explicit operator update | record itself, excluding embedded secrets |
