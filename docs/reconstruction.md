...
| Full application clean reconstruction | PASS for fresh Rocky generated-installer path | VM 802 installed the v2 generated records on Rocky 10.2; Hermes, Open WebUI, Hindsight, Grocy, SearXNG, Agent Zero, and LLDAP reached the tested acceptance boundary. |
...
| Fresh-install full application household soak | PASS for Alpha/Beta restart/isolation boundary | The generated-install path persisted Alpha's response, rejected Beta's private-chat read, preserved both outcomes across an Open WebUI restart, and the separate synthetic household soak covers memory/Grocy/web/operator composition; owner-visible authenticated composition remains separate. |

The installer-generated full application, household restart/isolation, and
generated-runtime reboot milestones are complete on the fresh Rocky guest. The
broader multi-domain household contract is separately evidenced by the
synthetic household soak. Installation/rebuild remains `PARTIAL` only for
owner-visible authenticated composition on a reconstructed deployment.
