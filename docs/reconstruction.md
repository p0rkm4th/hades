...
| Actual-image application composition | PASS | `scripts/test-full-application-image-startup.sh` starts the six pinned application images together on an isolated Docker network and verifies internal health; no host ports or production state are used. |
