...
`moby-engine`, `docker-compose`, `git`, and `openssl`, followed by
`sudo systemctl enable --now docker`. On Rocky 9/10, the helper installs
`git`/`openssl`, adds Docker's official CentOS-compatible RPM repository, and
installs `docker-ce`, `docker-ce-cli`, `containerd.io`, `docker-buildx-plugin`,
and `docker-compose-plugin`. The HADES installer validates these prerequisites
