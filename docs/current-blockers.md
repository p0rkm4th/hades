...
| Web/search owner follow-up | Credential-free API/CLI follow-up freshness and search-vs-page boundaries pass; owner-visible follow-up has not been accepted. | Run one authenticated owner-UI chain such as weather tomorrow → Sunday, verify a fresh search and grounded continuation, then retain the result in the private acceptance record. |
| Recipe URL/paste owner acceptance | Repository actor path, preview/apply boundary, public extraction, and disposable canonical Grocy read-back pass. | Deploy the private recipe MCP profile and complete one owner-visible preview → explicit confirmation → Grocy read-back sequence. |
| Shared Channels candidate promotion | Disposable Channels and candidate Open WebUI isolation evidence pass. | Complete the separate candidate promotion/owner UI acceptance sequence; production Open WebUI remains unchanged. |
