...
| Web search | PARTIAL | SearXNG with Hermes provider | SearXNG canonical queries and an initial owner weather turn pass; startup health checks now retry and live-web turns receive explicit re-search guidance. | A local-model follow-up previously answered from stale context without searching; the repair is deployed but needs another owner-UI follow-up acceptance. |
...
| Household / Grocy | PASS | Grocy + Hermes | HADES owner-API add, remove, correction/re-add, quantity handling, intake confirmation, consume, recipe fulfillment available/shortage branches, recipe add-missing merge, and restart persistence were verified; recipe authoring tools are now exposed for owner dogfood. | Recipe-authoring browser acceptance and additional pantry edge cases remain future hardening; Grocy remains authoritative. |
