...
| Durable memory | PARTIAL | Hindsight with Hermes memory provider | Owner UI retain/recall returned normalized Hindsight results; the correction branch exists in canonical Hindsight results and reload was verified. | Fresh-session generic recall can still surface an older retained paraphrase before the correction. Hindsight is contextual memory, not live domain truth. |
...
| Memory + household composition | PARTIAL | Hindsight + Grocy | A synthetic recipe-fixture fact was retained and recalled through HADES, combined with live Grocy fulfillment in one owner turn, and the correction branch was confirmed in canonical Hindsight results without changing the canonical Grocy recipe; mobile DOM and reload were verified. | Fresh-session generic recall can still surface an older retained paraphrase; Grocy remains authoritative for live state. |
