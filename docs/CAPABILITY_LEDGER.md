...
| Finance platform bakeoff | SELECT WITH CONDITIONS | Actual Budget provisional canonical platform; Firefly fallback; Finlynq/Ledgr watchlist | Four disposable synthetic candidates were started; Finlynq UI import/repeat-import exposed a likely same-merchant false-positive; stable Actual client/server synthetic import, repeat reconciliation, sync/reload, transfer-payee, and selected-archive restore proofs passed; native archive and Finlynq database restore probes passed. | Provider transfer normalization and owner promotion gates remain; no real finance data or provider is connected. |
| Finance read-only integration | PARTIAL / OWNER GATE | Actual Budget provisional canonical platform | A three-tool stdio MCP boundary over the official Actual API is implemented and exercised against synthetic canonical accounts, balances, transactions, server version, freshness metadata, and an unavailable-server failure; it exposes no finance mutation methods. | HADES owner-path conversation proof remains; real provider, accounts, environment, retention, and webhook authorization remain owner-gated. |
