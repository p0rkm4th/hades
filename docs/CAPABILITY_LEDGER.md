...
| Household / Grocy | PARTIAL | Grocy + Hermes | HADES owner-API add, remove, correction/re-add, bounded stock-intake preview plus explicitly confirmed apply, and canonical consume turns were verified against Grocy's API; restart persistence was also proven. | Broader pantry, recipe-shortage, and browser-DOM workflows remain open; local-model apply wording remains sensitive. |
