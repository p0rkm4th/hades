...
| Household / Grocy | PARTIAL | Grocy + Hermes | HADES owner-API add, remove, correction/re-add, and bounded stock-intake preview plus explicitly confirmed apply turns were verified against Grocy's canonical API; restart persistence was also proven. | Broader pantry, consumption, recipe-shortage, and browser-DOM workflows remain open; local-model apply wording remains sensitive. |
