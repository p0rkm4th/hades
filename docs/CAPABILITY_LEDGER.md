...
| Finance platform bakeoff | SELECT WITH CONDITIONS | Actual Budget provisional canonical platform; Firefly fallback; Finlynq/Ledgr watchlist | Four disposable synthetic candidates were started; Finlynq UI import/repeat-import exposed a likely same-merchant false-positive; Actual native archive and Finlynq database restore probes passed. | Actual-compatible import and a constrained HADES read adapter still require synthetic proof; no real finance data or provider is connected. |
| Finance read-only integration | PARTIAL / OWNER GATE | Actual Budget provisional canonical platform | Credential-independent read-only boundary, freshness/coverage rules, backup findings, and production gates are documented in `docs/adr/finance-platform.md`. | Synthetic HADES owner-path adapter proof remains; real provider, accounts, environment, retention, and webhook authorization remain owner-gated. |
