...
| Agent Zero delegation | PARTIAL | Agent Zero + Hermes MCP adapter | Official Agent Zero API contract and private one-tool MCP bridge are connected; a bounded owner-API delegation reached the adapter, but the test instance returned a safe upstream failure while Agent Zero was busy. | Keep the operator bounded and private; browser-DOM success and a clean retry remain open. |
