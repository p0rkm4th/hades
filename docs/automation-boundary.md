...
production. A private n8n runner now hosts one inactive, read-only canary; this
document defines the smallest safe contract for the first approved workflow.
It is not an automation engine or a permission system.
...
does not enable a schedule, grant control scopes, or promote a workflow to
production. The inactive private runner is not itself a production workflow.
